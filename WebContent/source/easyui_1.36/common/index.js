(function ($) {
    ...
    //      id: 表示根节点菜单的 id；    ./source/easyui_1.36/common/nav-api-menu-data.json    ./source/easyui_1.36/common/nav-doc-menu-data.json    
    window.mainpage.loadMenu = function (id) {
        ...
        if ($.array.contains(["10", "11"], id)) {
            $.get(id == 11 ? "./menu.do?startMenu" : "./menu.do?startMenu", function (menus) {
    ...
    window.mainpage.loadNavMenus = function (callback) {
        ...
        $.get("./source/easyui_1.36/common/nav-menu-data.json", function (menus) {
    ...
    window.mainpage.loadFavoMenus = function () {
        ...
        $(favoMenuTree).tree("load", "./source/easyui_1.36/common/favo-menu-data.json");
    };
        ...
        $(btnExit).click(function () {
            $.easyui.messager.confirm("操作提醒", "您确定要退出当前程序并关闭该网页？", function (c) {
				                if (c) {/* $.util.closeWindowNoConfirm(); */
				                	window.location.replace("./login.do?exit");
				                }
            });
        });
    };
    ...
    //初始化平台信息数据列表   ./source/easyui_1.36/common/donate-data.json
    $.util.namespace("donate");
    window.donate.reload = function () {
        var donatePanel = $("#donatePanel"), donate = $("#donateList").empty();
        $.ajax({
            url: "./permission.do?osInfo", type: "get", dataType: "json",
            beforeSend: function (XMLHttpRequest) { $.easyui.loading({ locale: donatePanel }); },
            success: function (data, textStatus, XMLHttpRequest) {
            	$("#info_1").addClass("donate-name").text("系统名称："+data.osName);
            	$("#info_2").addClass("donate-name").text("系统版本："+data.osVersion);
            	$("#info_3").addClass("donate-name").text("系统架构："+data.osArch);
            	$("#info_4").addClass("donate-name").text("jdk版本："+data.jdkVersion);
            	$("#info_5").addClass("donate-name").text("用户的当前工作目录："+data.projectPath);
            	$("#info_6").addClass("donate-name").text("Java 安装目录："+data.javaHome);
            	//$("#info_7").addClass("donate-name").text("载库时搜索的路径列表："+data.loadLibPath);
            	$("#info_8").addClass("donate-name").text("默认的临时文件路径："+data.tmpFile);
            	$("#info_9").addClass("donate-name").text("Java 运行时环境供应商："+data.javaVendor);
            	$("#info_10").addClass("donate-name").text("Java 供应商的 URL："+data.javaVendorUrl);
...
//                });
    ...
    //初始化友情链接列表   ./source/easyui_1.36/common/link-data.json
    $.util.namespace("link");
    window.link.reload = function () {
        var linkPanel = $("#linkPanel"), link = $("#linkList").empty();
        $.ajax({
            url: "./links.do?linkJSON&page=0&rows=100", type: "get", dataType: "json",
