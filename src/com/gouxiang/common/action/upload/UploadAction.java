package com.gouxiang.common.action.upload;
...
public class UploadAction {
	 ...
	 */
	@RequestMapping(params="picUpload",method=RequestMethod.GET)
	public String picAdd( @RequestParam MultipartFile[] photos, HttpServletRequest request, HttpServletResponse response) throws IOException{
		//可以在上传文件的同时接收其它参数
		...
		for(MultipartFile myfile : photos){
			if(myfile.isEmpty()){
	...
	@RequestMapping(params="fileUpload",method=RequestMethod.GET)
	public String addUser( @RequestParam MultipartFile[] files, HttpServletRequest request, HttpServletResponse response) throws IOException{
		//可以在上传文件的同时接收其它参数
				System.out.println("----------------------文件上传----------------------");
				//如果用的是Tomcat服务器，则文件会上传到\\%TOMCAT_HOME%\\webapps\\YourWebProject\\upload\\文件夹中
				//这里实现文件上传操作用的是commons.io.FileUtils类,它会自动判断/upload是否存在,不存在会自动创建
